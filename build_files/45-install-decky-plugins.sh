...
# Ship only the plugin runtime; the frontend build sources (node_modules, src,
# tooling configs) stay out of the image.
src=/ctx/decky/armada-control
cp -a "${src}/plugin.json" "${src}/package.json" "${src}/main.py" /usr/share/decky-plugins/ArmadaControl/
cp -a "${src}/py_modules" "${src}/dist" /usr/share/decky-plugins/ArmadaControl/
